...
function lade(){
  ...
  /* Übernahme aus der separaten Ankommens-Seite (früherer Speicherort) */
  if(!s){
    try{
      var a=JSON.parse(localStorage.getItem('ac_ich')||'null');
      if(a){ s={format:a.format||null, erklaerung:a.erklaerung||null, themen:a.themen||[], fertig:!!a.fertig};
        localStorage.setItem(KEY, JSON.stringify(s)); }
    }catch(e){}
  }
  if(s){
    /* ältere Werte vereinheitlichen */
    var MAP={wenig:'nein',mittel:'teils',viel:'ja'};
    if(MAP[s.erklaerung]) s.erklaerung=MAP[s.erklaerung];
  }
  return s;
}
function sichere(o){
...
window.AC_SETUP={
  ...
  format:function(){ var s=lade(); if(s&&s.format) return s.format;
    try{ var f=localStorage.getItem('ac_format'); if(f) return f; }catch(e){} return 'lang'; },
  erklaerung:function(){ var s=lade(); return (s&&s.erklaerung)||'teils'; },
